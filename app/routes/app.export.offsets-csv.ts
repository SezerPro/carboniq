...
import { exportOffsetsCsv } from "../lib/export/csv.server";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const csv = await exportOffsetsCsv(shop.id);

  return new Response(csv, {
    status: 200,
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="carboniq-offsets.csv"',
    },
  });
};
